...
final class CoordinatorContentFactory {
    ...
    static func create(for coordinatorType: CoordinatorType) -> CoordinatorContentInterface {
        ...
        case .window(let params):
            return CoordinatorWindowContent(window: params.window, showingType: params.showingType, rootViewController: params.rootChildCoordinator.content.viewController)
        case .tabBarController(let params):
            return CoordinatorTabBarControllerContent(showingType: params.showingType, viewControllers: params.rootChildCoordinators.map { $0.content.viewController })
